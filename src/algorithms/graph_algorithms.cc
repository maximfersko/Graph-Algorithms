...
graph_t GraphAlgoritms::GetLeastSpanningTree(graph_t &graph) {
    graph_t result;
    std::size_t gr_size = graph.GetSize();
    result.SetSize(gr_size);
    Matrix<unsigned int> res(gr_size, gr_size);
    std::vector<bool> selected(gr_size, false);
    std::size_t edge_count = 0;
    std::size_t x = 0;
    std::size_t y = 0;
    selected[0] = true;

    while (edge_count < gr_size - 1) {
        int min = std::numeric_limits<int>::max();
        for (size_t i = 0; i < gr_size; i++) {
            if (selected[i]) {
                for (size_t j = 0; j < gr_size; j++) {
                    if (!selected[j] && graph.GetAdjacencyMatrix()(i, j) > 0 &&
                        graph.GetAdjacencyMatrix()(i, j) < min) {
                        min = graph.GetAdjacencyMatrix()(i, j);
                        x = i;
                        y = j;
                    }
                }
    ...
    result.SetAdjacencyMatrix(&res);

    return result;
}

tsm_t GraphAlgoritms::SolveTravelingSalesmanProblem(graph_t &graph) {
    Colony col(graph);

    if (col.Resolvable() > 0) {
        throw std::runtime_error("The problem cannot be solved.");
    } else {
        col.FindBestTravel();
    }
